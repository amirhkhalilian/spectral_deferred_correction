...
for k_ind = 1:length(kvals)
	...
	%PlotResults(fig, u_true, u, t, t_all, u_sol_all, 120+tolind, kvals(k_ind), tol);
	tolind= tolind+1;
...
function PlotResults(fig, u_true, u, t, t_all, u_sol_all,pltind, k, tol)
	...
	xlabel('time t \in [0,1]'); title(sprintf('Jacobi Elliptic functions with initial k=%2.2f, tol=%1.1e',k,tol));
	set(gca,'FontSize',15);
